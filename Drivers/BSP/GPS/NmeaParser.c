...
//  GCHAR buffer[128];
  ...
  if (IsFloat(SubField[0]))
  {
    ...
    gga->fUtcSec = (GFLOAT)fmod(atof(SubField[0]),100.0);
  }
  else
  {
    gga->u2UtcHour = 0;
  ...
  {
    dfVal = atof(SubField[1]) / 100.0;
    gga->i2LatDeg = (GINT16)(dfVal);
    gga->i2LatMin = (GINT16)((dfVal - gga->i2LatDeg) * 100.0);
    gga->dfLatSec = (dfVal - gga->i2LatDeg - gga->i2LatMin * 0.01) * 6000.0;
    gga->dfLatitude = gga->i2LatDeg + gga->i2LatMin / 60.0 + gga->dfLatSec / 3600.0;
  }
  else
  {
    gga->i2LatDeg = 0;
...
//      gsa->u2SatID[i] = 0;
  ...
  {
    gsa->fPDOP = atof(SubField[IndexPDOP]);
  }
  else
  {
    gsa->fPDOP = 0.0;
  ...
  {
    gsa->fHDOP = atof(SubField[IndexHDOP]);
  }
  else
  {
    gsa->fHDOP = 0.0;
  ...
  {
    gsa->fVDOP = atof(SubField[IndexVDOP]);
  }
  else
  {
    gsa->fVDOP = 0.0;
...
GBOOL fgNmeaGPQSAParser(GCHAR* str, NMEA_QSA_T* qsa)
{
  ...
  if (IsFloat(SubField[IndexPDOP]))
  {
    qsa->fPDOP = atof(SubField[IndexPDOP]);
  }
  else
  {
    qsa->fPDOP = 0.0;
  ...
  if (IsFloat(SubField[IndexHDOP]))
  {
    qsa->fHDOP = atof(SubField[IndexHDOP]);
  }
  else
  {
    qsa->fHDOP = 0.0;
  ...
  if (IsFloat(SubField[IndexVDOP]))
  {
    qsa->fVDOP = atof(SubField[IndexVDOP]);
  }
  else
  {
    qsa->fVDOP = 0.0;
...
//  GINT32 i4Elv[4], i4Azi[4], i4Snr[4];
  ...
  for (i = 0; i < gsv->i2SatNum; i++)
  {
    ...
    {
      gsv->rSv[i].i2SNR = atof(SubField[6 + 4 * i]);
    }
    else
    {
      gsv->rSv[i].i2SNR = 0;
...
GBOOL fgNmeaGPRMCParser(GCHAR* str, NMEA_RMC_T* rmc)
{
      ...
      (i4NumArg == 13)) // TSI
  {
    if (IsFloat(SubField[0]))
    {
      ...
      rmc->fUtcSec = (GFLOAT)fmod(atof(SubField[0]),100.0);
    }
    else
    {
      rmc->u2UtcHour = 0;
...
GBOOL fgNmeaGPVTGParser(GCHAR* str, NMEA_VTG_T* vtg)
{
      ...
      (i4NumArg == 9))
  {
    if(IsFloat(SubField[0]))
    {
      vtg->fTrueHeading = atof(SubField[0]);
    }
    else
    {
    ...
    if(IsFloat(SubField[2]))
    {
      vtg->fMagHeading = atof(SubField[2]);
    }
    else
    {
      vtg->fMagHeading = 0.0;
    ...
    if(IsFloat(SubField[4]))
    {
      vtg->fHSpeedKnot = atof(SubField[4]);
    }
    else
    {
      vtg->fHSpeedKnot = 0.0;
